...
// Supports incremental updates on every tick for continuous market awareness.
...
class DeepMarketState {
   ...
   * @returns {Object|null} Deep market state object.
   */
    ...
    try {
      ...
      const state = {
        ...
        summary: {
          ...
          liquidityScore: 0.5, // will be updated by awareness
          regimeSuggestion: this._suggestRegime(adxData, rsiVal, bbWidth, atr),
        },
        // Confidence and reason (first‑class)
        confidence: 0,
        reason: '',
        status: 'confirmed', // 'developing' or 'confirmed'
      };
  ...
  _calculateConfidence(state) {
    ...
    if (state.awareness) {
      ...
      if (unusualEvents && unusualEvents.length > 0) conf -= 5; // uncertainty increases
    }
