...
router.post('/orders/command', async (req, res) => {
  try {
    ...
    if (!command.commandId) {
      // generate one if not provided (or let the adapter provide it)
      command.commandId = `cmd_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    }
    // Save to database (upsert if needed)
...
router.get('/orders/result/:commandId', async (req, res) => {
  try {
    ...
    if (result) {
      // Optionally delete after retrieval (or keep with TTL)
      // await Mt5CommandResult.deleteOne({ commandId });
      res.json(result);
    } else {
...
router.post('/positions', async (req, res) => {
  try {
    ...
    // Replace all positions for this login (delete old, insert new)
...
router.get('/positions', async (req, res) => {
  try {
    // Get latest login's positions (or all)
...
router.get('/trade/:ticket', async (req, res) => {
  try {
    ...
    // First search in current positions
    const position = await Mt5Position.findOne({ ticket: Number(ticket) }).lean();
    if (position) {
      return res.json(position);
    }
    // If not found, you might have a separate Trade history collection
...
router.get('/history', async (req, res) => {
  // You can implement using a Trade model (closed trades) from earlier
...
router.post('/sync', async (req, res) => {
  ...
  // Optionally clean old commands/results for this login
