...
// ---------- Trade History (unchanged – returns all trades) ----------
...
// ---------- Close Trade (FIXED: multi‑broker safe, string coercion, and logging) ----------
exports.closeTrade = async (req, res) => {
  const { tradeId } = req.params;
  if (!tradeId) return res.status(400).json({ error: 'tradeId required' });
  try {
    // 1. Fetch the trade to determine its broker
    const idStr = String(tradeId);
    const trade = await Trade.findOne({ contractId: idStr });
    if (!trade) {
      logger.warn(`[closeTrade] Trade not found: ${idStr}`);
      return res.status(404).json({ error: 'Trade not found' });
    }

    // 2. Determine the product to use for closing
    let product = null;
    if (trade.broker) {
      const brokerMap = {
        'MT5': 'mt5',
    ...
    // 4. Update the trade record (status, close price, P&L)
    ...
    if (!updated) {
      logger.warn(`[closeTrade] Trade ${idStr} not updated (maybe already closed).`);
    } else {
      ...
      // Update portfolio, notifications, learner
