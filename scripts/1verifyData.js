// verifyData.js – Inspect Candles, States, and Outcomes (Self‑Contained)
// Run: node scripts/verifyData.js
...
const OUTPUT_FILE = 'data_verification.json';
...
// ----- Define Candle model (self-contained, explicit collection) -----
const candleSchema = new mongoose.Schema({
  ...
  volume: Number,
  source: String,
}, { collection: 'historicalcandles' });
const Candle = mongoose.model('Candle', candleSchema);

// ----- Import State and Outcome models (your actual paths) -----
...
async function inspect() {
  ...
  console.log(`📂 HistoricalOutcome collection: ${HistoricalOutcome.collection.collectionName}`);

  // ---- 1. Candles ----
  printHeader('CANDLES (historicalcandles)');
  const totalCandles = await Candle.countDocuments();
  console.log(`Total candles: ${totalCandles}`);

  const candleStats = await Candle.aggregate([
  ...
  if (totalCandles > 0) {
    const sampleCandles = await Candle.find().limit(5).lean();
  ...
  let stateStats = [];
  if (totalStates > 0) {
    stateStats = await HistoricalState.aggregate([
      { $group: { _id: { symbol: '$symbol', timeframe: '$timeframe' }, count: { $sum: 1 } } },
      { $sort: { count: -1 } }
    ]);
    ...
    const sampleStates = await HistoricalState.find().limit(5).lean();
    ...
    const fullState = await HistoricalState.findOne({
      'outcome5.return': { $ne: null },
      'outcome10.return': { $ne: null },
      'outcome20.return': { $ne: null },
      'outcome40.return': { $ne: null },
  ...
  if (totalOutcomes > 0) {
    ...
    const sampleOutcomes = await HistoricalOutcome.find().limit(5).lean();
  ...
  const report = {
    candles: {
      ...
      sample: await Candle.find().limit(5).lean()
    },
    states: {
      total: totalStates,
      perSymbolTimeframe: stateStats,
      sample: await HistoricalState.find().limit(5).lean(),
      labelling: {
        outcome5: await HistoricalState.countDocuments({ 'outcome5.return': { $ne: null } }),
        outcome10: await HistoricalState.countDocuments({ 'outcome10.return': { $ne: null } }),
        outcome20: await HistoricalState.countDocuments({ 'outcome20.return': { $ne: null } }),
        outcome40: await HistoricalState.countDocuments({ 'outcome40.return': { $ne: null } }),
        fullyLabelled: await HistoricalState.countDocuments({
          'outcome5.return': { $ne: null },
          'outcome10.return': { $ne: null },
          'outcome20.return': { $ne: null },
          'outcome40.return': { $ne: null },
      ...
      fullyLabelledExample: await HistoricalState.findOne({
        'outcome5.return': { $ne: null },
        'outcome10.return': { $ne: null },
        'outcome20.return': { $ne: null },
        'outcome40.return': { $ne: null },
    ...
    outcomes: {
      total: totalOutcomes,
      perLookahead: await HistoricalOutcome.aggregate([
        { $group: { _id: '$lookahead', count: { $sum: 1 } } }
      ]),
      sample: await HistoricalOutcome.find().limit(5).lean()
    }
  };
  ...
  const fullyLabelled = report.states.labelling.fullyLabelled;
